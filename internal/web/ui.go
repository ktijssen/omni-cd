...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .cluster-card-title.clickable { cursor: pointer; }
  .cluster-card-title.clickable:hover { color: #FB326E; }
  .cluster-node-title { font-size: 11px; font-weight: 600; color: #71717a; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 8px; }
  .cluster-node-count { font-size: 22px; font-weight: 700; color: #fff; line-height: 1; margin-bottom: 4px; }
  .cluster-node-mc { font-size: 11px; color: #FB326E; margin-top: 4px; }
  .cluster-cp-section { padding: 12px 18px; border-top: 1px solid #3f3f46; display: flex; justify-content: space-between; align-items: center; }
  .cluster-cp-versions { text-align: right; }
  .cluster-cp-version-item { font-size: 13px; color: #a1a1aa; margin-bottom: 6px; }
  .cluster-cp-version-item:last-child { margin-bottom: 0; }
  .cluster-cp-version-item span { color: #e4e4e7; font-weight: 600; font-size: 14px; }
  .cluster-worker-col--empty { background: repeating-linear-gradient(135deg, transparent, transparent 4px, rgba(63,63,70,0.3) 4px, rgba(63,63,70,0.3) 5px); }
  .cluster-workers-row { display: flex; border-top: 1px solid #3f3f46; }
  .cluster-worker-col { flex: 1; padding: 12px 18px; min-width: 0; }
  .cluster-worker-col + .cluster-worker-col { border-left: 1px solid #3f3f46; }
  .diff-viewer {
    background: #18181b;
...
(function() {
  ...
  function renderClustersView(s) {
    ...
    var cards = clusters.map(function(c) {
      ...
      var workersToRender = workers.length > 0 ? workers : [{}];
      var workersHtml = '';
      for (var wi = 0; wi < workersToRender.length; wi += 2) {
        workersHtml += '<div class="cluster-workers-row">';
        for (var wj = wi; wj < Math.min(wi + 2, workersToRender.length); wj++) {
          var wk = workersToRender[wj];
          var title = wk.name || 'Workers';
          workersHtml += '<div class="cluster-worker-col">' +
            '<div class="cluster-node-title">' + title + '</div>' +
            '<div class="cluster-node-count">' + (wk.count || 0) + '</div>' +
            (wk.machineClass ? '<div class="cluster-node-mc">' + wk.machineClass + '</div>' : '<div style="font-size:11px;color:#52525b">Static nodes</div>') +
          '</div>';
        }
        // Pad last row with empty block if odd number of workers
        if (wi + 2 > workersToRender.length && workersToRender.length % 2 !== 0) {
          workersHtml += '<div class="cluster-worker-col cluster-worker-col--empty"></div>';
        }
        workersHtml += '</div>';
      }
      return '<div class="cluster-card">' +
        '<div class="cluster-card-header">' +
          '<span class="cluster-card-title' + (hasDetails ? ' clickable' : '') + '"' +
        ...
        '<div class="cluster-cp-section">' +
          '<div>' +
            '<div class="cluster-node-title">Controlplane</div>' +
            '<div class="cluster-node-count">' + cpCount + '</div>' +
            (cpMC ? '<div class="cluster-node-mc">' + cpMC + '</div>' : '<div style="font-size:11px;color:#52525b">Static nodes</div>') +
          '</div>' +
          '<div class="cluster-cp-versions">' +
            '<div class="cluster-cp-version-item">Talos <span>' + (c.talosVersion || '-') + '</span></div>' +
            '<div class="cluster-cp-version-item">Kubernetes <span>' + (c.kubernetesVersion || '-') + '</span></div>' +
          '</div>' +
        '</div>' +
        workersHtml +
      '</div>';
  ...
  function renderModal() {
    return '<div class="modal ' + (currentModal ? 'show' : '') + '" onclick="if(event.target === this) window.__closeModal()">' +
  ...
  function render() {
    ...
    app.innerHTML =
      ...
      '<div class="panels">' +
        ...
        '</div>' +
        '<div class="panel">' +
          '<div class="panel-header"><span class="panel-nav-link" onclick="window.__showClustersView()">Clusters</span> ' +
            '<div class="panel-header-right">' +
